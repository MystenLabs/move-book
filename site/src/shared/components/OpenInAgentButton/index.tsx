...
> = {
  ...
  build: (code, lang) =>
    [
      "Use this prompt for building with Move:",
      `\n\n\`\`\`${lang}\n${code}\n\`\`\``,
    ].join(""),
};
...
/** Walk up from `start` to find the nearest `data-agent-prompt` attr. */
function getNearestPromptPreset(
...
function buildPrompt(
  ...
  const fn =
    PROMPT_PRESETS[preset] ?? PROMPT_PRESETS[DEFAULT_PRESET];
  return fn(code, lang);
}
...
export default function OpenInAgentButton({
  ...
  const handleAgentClick = useCallback(
    (agent: AgentItem) => {
      ...
      const preset = getNearestPromptPreset(
        wrapperRef.current,
      );
      if (!code) return;
      window.open(
        agent.url(code, lang, preset),
        "_blank",
        "noopener",
      );
      setIsOpen(false);
